...
struct ADSR {
    ...
    ADSR() : attack(0.01f), decay(0.3f), sustain(0.0f), release(1.5f) {
    };
};
...
struct Harmonic {
    ...
    WaveType waveType; // 该泛音的波形图
    ADSR adsr; // 该泛音的包络数据
    ToneDressUp toneDressUp; // 该泛音的细节修饰

    Harmonic(
        const float ratio,
        const float amplitude,
        const WaveType wt = WaveType::Sine,
        const ADSR &env = ADSR(),
        const ToneDressUp &dress = ToneDressUp()
    )
        : freqRatio(ratio), amp(amplitude), waveType(wt), adsr(env), toneDressUp(dress) {
    }
    Harmonic() {
        freqRatio = 1.0f;
        amp = 1.0f;
...
struct TonePreset {
    ...
    TonePreset(const string &pitchName, const Flavor &flavorName)
        : fundamentalPitch(pitchName),
          fundamentalFrequency(fundamentalPitch.get_frequency()),
          waveType(WaveType::Sine),
          adsr(ADSR(0.01f, 0.3f, 0.0f, 1.5f)),
