...
#define RESULT_PATH "../external/SSP-MMC/result/"

double SSP_MMC::read_SSPMMC_Result(const std::string& type, const std::string& difficulty, const double memoryState) {
    // 拼接逻辑连接
    ...
    while (std::getline(csv, line)) {
        ...
        // double data = std::stod(line.substr(pos + 1));

        // 方案2:
        // double memoryState, data;
        // sscanf(line.c_str(), "%lf,%lf", &memoryState, &data);

        // 方案3:
        double _memoryState, _data;
        std::istringstream iss(line);
        if (char comma = 0; !(iss >> _memoryState >> comma >> _data) || comma != ',') {
    ...
    // 收回兜底
    if (data < 0) {
        throw std::runtime_error("Invalid data: " + std::to_string(data));
    }
    return data;
}
